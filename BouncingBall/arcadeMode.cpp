...
void arcadeMode(int switch_mode)
{
	...
	*/
	...
	while (ArcadeWindow.isOpen())
	{
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::S) && (paddle.getPosition().y < WINDOW_HEIGHT - 35)) {
			paddle.move({ 0, padSpeed * timeElapse });
		}
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::A) && (paddle.getPosition().x > 75)) {
			paddle.move({ -padSpeed * timeElapse, 0 });
		}
		...
		if (start == false)
		{
			ball.setPosition({ paddle.getPosition().x + paddle.getSize().x / 2 - ball.getRadius(), paddle.getPosition().y - 20 });
		...
		if (status != 0)
		{
			...
			if (switch_mode == 1)
			{
				high_score.saveHighScore(Score, "Data/highscore_speed_mode.txt");
			}
			else if (switch_mode == 2)
			{
				high_score.saveHighScore(Score, "Data/highscore_level_mode.txt");
			}
